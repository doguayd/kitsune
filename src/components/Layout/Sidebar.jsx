...
export default function Sidebar() {
  ...
  return (
    <aside
      className="w-52 flex flex-col shrink-0 bg-panel"
    ...
    >
      ...
      >
        ...
        <p className="text-cream text-sm font-medium mt-2">{user?.name ?? 'Tilki'}</p>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors
               ${isActive
                 ? 'bg-fox/20 text-fox font-medium'
      ...
      <div className="p-3 space-y-1.5" style={{ borderTop: '1px solid #2a2a4a' }}>
        ...
        <Dot label="TTS (XTTS)" on={ttsOnline} />
      </div>
    </aside>
  )
}
